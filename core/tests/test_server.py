...
def test_parse_peers_handles_whitespace_and_empty() -> None:
    """`parse_peers` is consumed at server boot to materialize the static
    peer registry from `OTEL_A2A_RELAY_PEERS`. Pin the parser since an
    off-by-one whitespace bug here would silently drop legitimate peers."""
    from otel_a2a_relay_core.server import parse_peers
...
def test_classify_relay_reject_covers_all_buckets() -> None:
    ...
    from otel_a2a_relay_core.server import _classify_relay_reject

...
def header(ax, x, y, label, port, edge):
    ...
    if port:
        ax.text(
            x, y - 0.42, port,
            ha="center", va="center",
            fontsize=10.5, color=edge, family="monospace", zorder=3,
        )
...
def span_lines(ax, x, y, lines):
    for i, (name, kind) in enumerate(lines):
        ax.text(
            ...
            name,
            ha="center", va="center",
            fontsize=10, color=INK, family="monospace", zorder=3,
        )
        ax.text(
            x, y - i * 0.42 - 0.22,
            kind,
            ha="center", va="center",
            fontsize=8.5, color=MUTE, style="italic", zorder=3,
        )
...
def arrow(ax, x0, x1, y, label_top, label_bottom):
    ...
    midx = (x0 + x1) / 2
    ax.text(
        midx, y + 0.32, label_top,
        ha="center", va="bottom",
        fontsize=10.5, color=INK, family="monospace", fontweight="bold",
    )
    ax.text(
        midx, y - 0.34, label_bottom,
        ha="center", va="top",
...
def main():
    ...
    span_lines(ax, cx_client, box_top - 1.55, [
        ("a2a.client.send", "CLIENT"),
    ])

    # relay (slightly taller emphasis via thicker edge)
    rounded_box(
        ax, cx_relay - box_w / 2, box_y, box_w, box_h,
        RELAY_FILL, RELAY_EDGE, lw=2.2,
    )
    header(ax, cx_relay, box_top - 0.45, "relay", ":8080", RELAY_EDGE)
    span_lines(ax, cx_relay, box_top - 1.55, [
        ("a2a.task", "SERVER"),
    ...
    span_lines(ax, cx_peer, box_top - 1.55, [
    ...
    ax.plot(
        ...
        [line_y, line_y],
        color=TRACE_LINE, linewidth=1.6, linestyle=(0, (4, 3)), zorder=1,
    )
    ax.text(
        6.5, line_y - 0.32,
        "one W3C trace_id end-to-end",
        ha="center", va="top",
        fontsize=10.5, color=TRACE_LINE, fontweight="bold",
    )

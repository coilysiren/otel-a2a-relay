...
from otel_a2a_relay.luca import deployer as deployer_mod
from otel_a2a_relay.luca.orchestrator import run_flow
...
def main() -> int:
    ...
    p.add_argument(
        "--root",
        default=str(Path(__file__).resolve().parents[3] / "examples" / "luca-flow"),
        help="examples/luca-flow root",
    )
    p.add_argument("--phoenix", default="http://localhost:6006")
    p.add_argument("--require-phoenix", action="store_true", default=True)
    p.add_argument("--no-require-phoenix", dest="require_phoenix", action="store_false")
    args = p.parse_args()
    ...
    if args.require_phoenix:
        try:
            r = httpx.get(f"{args.phoenix}/healthz", timeout=2.0)
            if r.status_code != 200:
                raise RuntimeError(f"phoenix healthz {r.status_code}")
            print(f"📡 Phoenix is up at {args.phoenix}")
        except httpx.HTTPError as e:
            print(
                f"❌ Phoenix not reachable at {args.phoenix}/healthz ({e}). "
                "Start it via `make phoenix-fg` (operator) or pass --no-require-phoenix.",
                file=sys.stderr,
            )
            return 3
    ...
    env = os.environ.copy()
    env["OTEL_EXPORTER_OTLP_ENDPOINT"] = args.phoenix
    ...
    try:
        _spawn(
            ...
            [
                ...
                "run",
                "uvicorn",
                "otel_a2a_relay.server:create_app",
                "--factory",
                "--host",
                "127.0.0.1",
                "--port",
